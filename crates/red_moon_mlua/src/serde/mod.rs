...
pub trait LuaSerdeExt: Sealed {
    ...
    /// use red_moon_mlua::{Lua, Result, LuaSerdeExt};
    ///
    /// fn main() -> Result<()> {
    ///     let lua = Lua::new();
    ...
    ///
    /// ```
    /// use red_moon_mlua::{Lua, Result, LuaSerdeExt, SerializeOptions};
    ///
    /// fn main() -> Result<()> {
    ///     let lua = Lua::new();
    ///     let v = vec![1, 2, 3];
    ...
    ///
    /// ```
    /// use red_moon_mlua::{Lua, Result, LuaSerdeExt, DeserializeOptions};
    /// use serde::Deserialize;
    ///
    /// #[derive(Deserialize, Debug, PartialEq)]
    /// struct User {
...
impl LuaSerdeExt for Lua {
    ...
    fn array_metatable(&self) -> Table {
        Table {
            lua: self,
            table_ref: self.array_metatable_ref().clone(),
        }
    }
